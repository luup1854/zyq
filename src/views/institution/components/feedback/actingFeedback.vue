...
import { ref } from 'vue'
import { getstuActivityIngList, updateActIngFeeback } from '@/api/zyb/institution'
...
const init = async () => {
	...
	const res = await getstuActivityIngList(userInfo.id)
...
const dialogChange = async () => {
	...
	ElMessageBox.confirm('是否要提交?', '提示', {
		...
		const res = await updateActIngFeeback(info.value)
		if (res.code === 0) {
