...
import { ref } from 'vue'
import { getstuJobList, updateJobFeeback } from '@/api/zyb/institution'
...
const init = async () => {
	infoList.value = []
	// todo 需要从storage中获取id
	const userInfo = JSON.parse(getUserInfo())
	info.value.id = userInfo.id
	const res = await getstuJobList(info.value.id)
...
const dialogChange = async () => {
	...
	ElMessageBox.confirm('是否要提交?', '提示', {
		...
		const res = await updateJobFeeback(info.value)
		if (res.code === 0) {
